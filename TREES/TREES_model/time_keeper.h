...
class Time
{
	...
	//input functions - to be modified/added to if needed
		...
		friend istream& read_time(istream& in, Time& tm);

		//from time stamp output - yyyy:ddd:hh:mm (not fixed width)
		friend istream& read_time_stamp(istream& in, Time& tm);

		//the format is mm.dd	hh.mm	yday.fractionday
		//no year info so year is in argument
		friend istream& read_time_tdat(istream& in, Time& tm, int year);
...
class Time_Array
{
	public:
		...
		friend istream& read_step(istream& in, Time_Array& ta, int step);
                friend ostream& print_step(ostream& out, const Time_Array& ta, int step);
	private:
		Time* series;
		int steps;
};
